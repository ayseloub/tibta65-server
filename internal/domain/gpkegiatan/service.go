...
const uploadFolder = "gp-kegiatan"

type ListResult struct {
	Items      []GPKegiatan `json:"items"`
	Page       int          `json:"page"`
...
type Service interface {
	...
	Get(ctx context.Context, slug string) (*GPKegiatan, error)
	Create(ctx context.Context, in CreateInput) (*GPKegiatan, error)
	Update(ctx context.Context, in UpdateInput) (*GPKegiatan, error)
...
func (s *service) List(ctx context.Context, f ListFilter) (*ListResult, error) {
	...
	return &ListResult{Items: items, Page: f.Page, Limit: f.Limit, Total: total, TotalPages: totalPages}, nil
}

func (s *service) Get(ctx context.Context, slugParam string) (*GPKegiatan, error) {
	return s.repo.FindBySlug(ctx, slugParam)
}

func (s *service) Create(ctx context.Context, in CreateInput) (*GPKegiatan, error) {
	if in.Title == "" || in.Date == "" || in.KordaID == "" || in.Location == "" || in.Description == "" {
	...
	var kategoriIDPtr *string
	if in.KategoriID != "" {
		kategoriIDPtr = &in.KategoriID
	}

	k := &GPKegiatan{
		ID: ulid.Make().String(), Slug: slug.Generate(in.Title), Title: in.Title, Date: date,
		KordaID: in.KordaID, KategoriID: kategoriIDPtr, Location: in.Location, ImageURL: imageURL,
		Description: in.Description, PublishAt: in.PublishAt, ExpireAt: in.ExpireAt,
	}
...
func (s *service) Update(ctx context.Context, in UpdateInput) (*GPKegiatan, error) {
	if in.Title == "" || in.Date == "" || in.KordaID == "" || in.Location == "" || in.Description == "" {
		return nil, ErrValidation
	}
	if in.PublishAt != nil && in.ExpireAt != nil && !in.ExpireAt.After(*in.PublishAt) {
	...
	k := &GPKegiatan{
		Slug: in.Slug, Title: in.Title, Date: date, KordaID: in.KordaID, KategoriID: kategoriIDPtr,
		Location: in.Location, ImageURL: imageURL, Description: in.Description,
