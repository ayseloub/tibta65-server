...
func (h *Handler) Create(c echo.Context) error {
	...
	return response.Success(c, http.StatusCreated, "Korda berhasil ditambahkan", result)
}
...
func (h *Handler) Update(c echo.Context) error {
	...
	return response.Success(c, http.StatusOK, "Korda berhasil diperbarui", result)
}
...
func (h *Handler) Delete(c echo.Context) error {
	...
	return response.Success(c, http.StatusOK, "Korda berhasil dihapus", nil)
}
...
func handleError(c echo.Context, err error) error {
	...
	case errors.Is(err, ErrDuplicate):
		return response.Error(c, http.StatusConflict, "Nama korda sudah dipakai")
	case errors.Is(err, ErrNotFound):
		return response.Error(c, http.StatusNotFound, "Korda tidak ditemukan")
	case errors.Is(err, ErrInUse):
		return response.Error(c, http.StatusConflict, "Korda masih dipakai oleh kegiatan lain")
	default:
		return response.Error(c, http.StatusInternalServerError, "Terjadi kesalahan pada server")
	}
}
...
func RegisterRoutes(e *echo.Echo, h *Handler, jwtSecret string) {
	e.GET("/api/kordas", h.List)

	adminGroup := e.Group("/api/admin/kordas",
